...
func NewConsulInstance(
	...
	address string, port int, ttl string, aclToken string, caFile, keyFile, certFile string,
) (*Instance, error) {
	hash := fnv.New32a()
	...
	client, err := base.NewClient(consulAddress, aclToken, caFile, keyFile, certFile)
	if err != nil {
		return nil, err
	}
